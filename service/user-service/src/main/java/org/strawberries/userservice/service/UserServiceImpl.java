...
public class UserServiceImpl implements UserService {
    ...
    public UserResponse create(UserRequest request) {
        if (request.email() != null && repository.existsByEmailAndIsActiveTrue(request.email())) throw new AttributeAlreadyExistsException("email", request.email());
        if (repository.existsByPhoneAndIsActiveTrue(request.phone())) throw new AttributeAlreadyExistsException("phone", request.phone());
    ...
    public PagedResponse<UserResponse> findAll(int page, int size, Boolean isActive) {
        ...
        Page<User> userPage = isActive == null ? repository.findAll(pageable) : repository.findAllByIsActive(pageable, isActive);
    ...
    public UserResponse findById(UUID id, boolean onlyActive) {
        Optional<User> user = onlyActive ? repository.findByIdAndIsActiveTrue(id) : repository.findById(id);
    ...
    public UserResponse update(UUID id, UserRequest request) {
        ...
        if (!user.getEmail().equals(request.email()) && repository.existsByEmailAndIsActiveTrue(request.email()))
            throw new AttributeAlreadyExistsException("email", request.email());
        if (!user.getPhone().equals(request.phone()) && repository.existsByPhoneAndIsActiveTrue(request.phone()))
    ...
    public UserResponse patch(UUID id, UserPatchRequest request) {
        ...
        if (request.email() != null && !request.email().equals(user.getEmail()))
            if (repository.existsByEmailAndIsActiveTrue(request.email()))
                throw new AttributeAlreadyExistsException("email", request.email());
        if (request.phone() != null && !request.phone().equals(user.getPhone()))
            if (repository.existsByPhoneAndIsActiveTrue(request.phone()))
                throw new AttributeAlreadyExistsException("phone", request.phone());
